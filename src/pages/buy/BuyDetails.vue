  ...
  <transition name="buy" :key="id" mode="out-in">
    <section>
      <div class="prev_next">
        ...
        <router-link
          class="next"
          v-if="id < Object.keys(buyProperties).length - 1"
          :to="next"
      ...
      <div class="home_container">
        <div class="info_container">
          <h1>{{ buyProperties[id].projectName }}</h1>
          <h3>{{ buyProperties[id].address }}</h3>
          <div class="info">
            <div class="under">
              <p>Price</p>
              <h5>{{ buyProperties[id].price }}</h5>
            </div>
            <div class="under">
              <p>Neighborhood</p>
              <h5>{{ buyProperties[id].neighborhood }}</h5>
            </div>
            <div class="under">
              <p>Sqft</p>
              <h5>{{ buyProperties[id].sqft }}</h5>
            </div>
            <div class="under">
              <p>Floor</p>
              <h5>{{ buyProperties[id].floor }}</h5>
            </div>
            <div class="under">
              <p>Total Rooms</p>
              <h5>{{ buyProperties[id].totalRooms }}</h5>
            </div>
          </div>
        </div>
        <img :src="buyProperties[id].bigImage" />
        <div class="about">
          ...
          <div class="icons">
            <div
              class="icon"
              v-for="amenity in buyProperties[id].amenitiesInfo"
              :key="amenity.amenity"
            >
...
export default {
  ...
  computed: {
    ...
    next() {
      return this.id < Object.keys(this.buyProperties).length - 1
        ? `/Buy/${Number(this.id) + 1}`
        : '';
    },

    prev() {
      return this.id > 0 ? `/Buy/${Number(this.id) - 1}` : '';
    },
  },
};
</script>
